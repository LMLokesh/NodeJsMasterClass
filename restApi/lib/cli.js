...
cli.responders.help = () =>{
    let commands = {
        ...
        'list users': 'Show a list of all the registered (undeleted) users in the stestem',
        'more user info --{userId}': 'Show details of a specific user',
        'list checks --up --down': 'Show a list of all the active checks in the system, including their state. The "--up" and "--down" are both optional',
        'more check info --{checkId}': 'Show details of a specified check',
        'list logs': 'Show a list of all the log files available to be read (compressed only)',
        'more log info --{fileName}' : 'Show detials of a specified log file'   
    };

    // show a header for the help page that is as wide as the screnn
    cli.horizontalLine();
    cli.centered("CLI MANUAL");
    cli.horizontalLine();
    cli.verticalSpace(2);

    // Show each command, folowed by its explanation, in white and yellow respectively 
...
cli.centered = (str)=>{
    ...
    // Get the availabe screen size
    let width = process.stdout.columns;

    // Calculate the left padding there sould be
...
cli.processInput = (str) =>{
    ...
    if(str) {
        // Codify the unique strings that identify the unique questions allowed to be ask
        let uniqueInputs = [
            'man',
            'help',
            'exit',
